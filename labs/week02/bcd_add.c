...
big_bcd_t *bcd_add(big_bcd_t *x, big_bcd_t *y) {
/*    big_bcd_t *sum = malloc(sizeof(*pointer));
    int longer = (x->n_bcd >= y->n_bcd) ? x->n_bcd : y->n_bcd;
    int shoter = x->n_bcd + y->n_bcd - longer;
    unsigned char *l = (longer == x->n_bcd) ? x->bcd ; y->bcd;
    unsigned char *s = (longer == x->n_bcd) ? y->bcd ; x->bcd; 
